...
def __main():
    ...
    graph_file = os.path.join(os.path.dirname(__file__), '..', 'data', 'com-youtube.ungraph.txt')
    ...
    parser.add_argument('--delta', type=float, help='Minimum distance parameter for near duplicate communities',
                        default=0.2)
    ...
    communities = growclusters(G, seeds, args.expansion, args.stopping, args.nworkers, args.nruns, args.alpha,
                               args.maxexpand, False)
    ...
    print("Communities are written to output file:")
    save_output(list(communities), os.path.basename(graph_file))
